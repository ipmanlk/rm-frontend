...
const showNewEntryModal = () => {
	mainForm.reset();
	$("#modalMainFormTitle").text("Add New Job Category");
	$("#modalMainForm").modal("show");
};

const showEditEntryModal = async (id, readOnly = false) => {
	// get entry data from db and show in the form
	const response = await Request.send("/api/job_categories", "GET", {
		data: { id: id },
	});
	...
	if (readOnly) {
		$("#modalMainFormTitle").text("View Job Category");
		mainForm.enableReadOnly();
	} else {
		$("#modalMainFormTitle").text("Edit Job Category");
...
const addEntry = async () => {
	...
	// get response
	const response = await Request.send("/api/job_categories", "POST", {
		data: data,
	});
...
const updateEntry = async () => {
	...
	const response = await Request.send("/api/job_categories", "PUT", {
		data: newEntryObj,
	});
...
const deleteEntry = async (id = mainForm.selectedEntry.id) => {
	...
	if (confirmation) {
		const response = await Request.send("/api/job_categories", "DELETE", {
			data: { id: id },
		});
		if (response.status) {
			reloadModule();
